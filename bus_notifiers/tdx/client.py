...
def get_estimated_time_of_arrive(route_name, top=30, skip=0):
    url = f"https://tdx.transportdata.tw/api/basic/v2/Bus/EstimatedTimeOfArrival/City/Taipei/{route_name}"
    return _get(url, top=top, skip=skip)


def get_estimated_time_of_arrive_with_stop(
    route_name, stop_name, direction, top=30, skip=0
):
    url = f"https://tdx.transportdata.tw/api/basic/v2/Bus/EstimatedTimeOfArrival/City/Taipei/{route_name}"
